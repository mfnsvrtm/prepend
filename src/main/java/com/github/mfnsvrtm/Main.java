...
class Args {
    @Parameter(names = {"-t", "--targets"}, description = "path to target list")
    Path fileListPath = Path.of("targets.txt");

    @Parameter(names = {"-c", "--copyright"}, description = "path to copyright notice")
    Path copyrightNoticePath = Path.of("copyright.txt");

    @Parameter(names = {"-b", "-r", "--root-dir", "--base-dir"}, description = "root directory for relative targets")
    Path rootDirectoryPath = null;

    @Parameter(names = {"-a", "--add-lines"}, description = "number of empty lines to be appended to the notice",
            validateValueWith = NonNegativeIntegerValidator.class)
    Integer extraLineCount = 0;

    @Parameter(names = {"-e", "--line-ending"}, description = "line ending of the empty lines created by --add-lines")
